CREATE OR REPLACE VIEW public."vw_conceptual_model_variables_adjpd" AS
 WITH hospital_data AS (
         ...
         SELECT hospital_data.county_fips,
            COALESCE(
                CASE
                    WHEN sum(hospital_data.total_adjpd) > 0::numeric THEN sum(hospital_data.ai_adoption_score * hospital_data.total_adjpd) / sum(hospital_data.total_adjpd)
                    ELSE NULL::numeric
                END, 0::numeric) AS weighted_ai_adoption_score,
            COALESCE(
                CASE
                    WHEN sum(hospital_data.total_adjpd) > 0::numeric THEN sum(hospital_data.robotics_adoption_score * hospital_data.total_adjpd) / sum(hospital_data.total_adjpd)
                    ELSE NULL::numeric
